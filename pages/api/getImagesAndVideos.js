...
async function connectToDatabase() {
  ...
  return {
    client,
    collection: client
      .db("images-and-videos-DB")
      .collection("images-and-videos"),
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === "POST") {
      ...
      upload.single("file")(req, res, async (err) => {
        ...
        try {
          ...
          res
            .status(200)
            .json({ message: "Media item submitted successfully!" });
        } catch (error) {
    ...
    // Close the MongoDB client connection after handling the request
    if (client) {
      await client.close();
    }
  }
